  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/v1/orders', {
        ...
        products: [{ product: formData.productId, quantity: parseInt(formData.quantity) }],
        status: formData.status
      });
      setResponse(res.data);
      setFormData({ userId: '', productId: '', quantity: '', status: 'pending' });
    } catch (err) {
      ...
      <div className="mx-auto max-w-2xl">
        ...
        <div className="bg-slate-900/70 backdrop-blur-sm rounded-2xl border border-slate-800 shadow-2xl p-8">
          <div className="mb-6">
            ...
            <p className="text-slate-400">POST /api/v1/orders</p>
          </div>
